/* ══════════════════════════════════════════════════════════════════════════
   Mar-A-Lavitch — static demo mode.
   ...
   of the demo dataset, so pool-manager.html and worker.html run unmodified.
   ...
   someone in and watch it stick — on their machine only. "Reset demo" in the
   banner wipes it back to the seed.
   ══════════════════════════════════════════════════════════════════════════ */
(function (global) {
'use strict';

if (typeof global.DEMO_SEED !== 'function') {
  console.warn('[demo] demo-data.js did not load — leaving fetch alone.');
  return;
}

/* ── Should we take over? ────────────────────────────────────────────────
   Two ways in. The fast path is for hosts we know have no API — Pages,
   file://, or an explicit ?demo=1 — and is decided synchronously so the page
   ...
   we switch to demo. A live server answers normally and this file gets out
...
let mode = STATIC_HOST ? 'demo' : 'unknown';   // 'demo' | 'live' | 'unknown'
...
function save() {
  ...
  catch (e) { /* storage full or blocked: the demo still works for this page */ }
}
function resetDemo() {
...
const routes = {
  ...
  '/api/auth': b => {
    ...
    if (!emp.password) return ok();                    // demo staff have none
  ...
  '/api/punch/out': b => {
    ...
    open.checkout_photo = null;                       // demo drops the photo
    save();
...
/* ── Answering out of the demo store ─────────────────────────────────────*/
function serveLocally(path, method, init) {
  if (method === 'GET') {
    ...
    return fail('Unknown demo route', 404);
  }
  ...
  try {
    ...
    console.error('[demo] handler failed for', path, err);
    return fail('Demo handler error');
  }
}
...
global.fetch = function (input, init) {
  ...
  if (mode === 'demo') return Promise.resolve(serveLocally(path, method, init));
  ...
  return realFetch(input, init).then(res => {
    ...
    activateDemo('404 from ' + path);
    return serveLocally(path, method, init);
  }, err => {
    activateDemo(err && err.message ? err.message : 'request failed');
    return serveLocally(path, method, init);
  });
};
